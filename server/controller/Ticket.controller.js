...
exports.createTicket = async (req, res) => {
  try {
    ...
    if (!numberOfTickets || numberOfTickets <= 0) {
      return res
        .status(400)
        .json({ message: "Missing or invalid number of tickets" });
    }
...
const sendEmail = async (email, subject, ticket) => {
  ...
  const transporter = nodemailer.createTransport({
    ...
    auth: {
      ...
      pass: process.env.EMAIL_PASSWORD, // App Password هنا
    },
  });
...
`;
  ...
  try {
    ...
    console.error("Email error details:", err); // أضف هذا
    throw new Error(`Email sending failed: ${err.message}`); // وهذا
  }
};
...
exports.scanTicket = async (req, res) => {
  try {
    ...
    if (ticket.paid === false) {
      return res
        .status(200)
        .json({ message: "This ticket has not been paid for yet." });
    }
...
exports.markTicketAsPaid = async (req, res) => {
  ...
  try {
    ...
    if (ticket.paid === true) {
      return res
        .status(200)
        .json({ message: "Ticket is already marked as paid." });
    }
